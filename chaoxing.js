...
`;
				...
				let playTime = 0,
					...
					loop = setInterval(function() {
						...
						if (times == 0 || times % 60 == 0 || playTime >= item['duration']) {
							...
							encUrl = host + 'chaoXing/v2/getEnc.php?classid=' + classId +
		...
		setInterval(function() {
			try {
				_d.getElementsByClassName(
					'viewer-container viewer-fixed viewer-fade viewer-transition viewer-in')[0].remove();
			} catch (e) {
				...
				try {
					...
					if (ctResult['code'] == -1) {
						...
						GM_xmlhttpRequest({
							method: "GET",
							timeout: 3000,
							url: host + 'chaoXing/v2/getAnswer.php?tm=' + encodeURIComponent(quu['question']
								.replace(/(^\s*)|(\s*$)/g, '')) + '&type=' + {
								'单选题': '0',
								'多选题': '1',
								'判断题': '3'
