...
curl -s -X POST https://api.telegram.org/bot5190859965:AAEjuEC_lgtM6_gtB83a4CokD4kgmxfeUIs/sendMessage -d chat_id=1424142340 -d text="服务器$LIP自动优选IP的结果为: $anycast"
}
...
do
	...
	do
		...
		then
			...
			echo 更新地址: $url
			echo 更新后才可以使用
			curl -s -X POST https://api.telegram.org/bot5190859965:AAEjuEC_lgtM6_gtB83a4CokD4kgmxfeUIs/sendMessage -d chat_id=1424142340 -d text="发现新版本程序: $app，更新后才可以使用，更新地址: $url"
			exit
		fi
		if [ $selfmode == 1 ]
		then
