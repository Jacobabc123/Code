...
class MetronPay extends AbstractPayment
{
    public function purchase($request, $response, $args, $telegram = 0)
    {
        ...
        if ($type == 'alipay') {
            ...
            switch ($payment_system) {
                ...
                case ('ZYGPay'):
                    $ZYGPay = new ZYGPay();
                    $result = $ZYGPay->MetronPay($type, $price, $shopinfo, $paylist_id);
                    if ($result['errcode'] === 0) {
                        $return = array(
                            'ret' => 1,
                            'type' => 'url',
                ...
                case ('ZYGPay_1'):
                    ...
                    $result = $ZYGPay_1->MetronPay($type, $price, $shopinfo, $paylist_id);
                    if ($result['errcode'] === 0) {
                        $return = array(
                            'ret' => 1,
                            'type' => 'url',
    ...
    public function notify($request, $response, $args)
    {
        ...
        switch ($payment_system) {
            ...
            case ('ZYGPay'):
                    $ZYGPay = new ZYGPay();
                    $result = $ZYGPay->notify($request, $response, $args);
                    return;
            case ('ZYGPay_1'):
                    $ZYGPay_1 = new ZYGPay_1();
                    $result = $ZYGPay_1->notify($request, $response, $args);
                    return;
            case ('bobpay'):
                $tron = new BobTronPay();
