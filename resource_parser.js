...
//http=example.com:443, username=name, password=pwd, over-tls=true, tls-host=example.com, tls-verification=true, tls13=true, fast-open=true, udp-relay=true, tag=http-tls-02
...
function HPS2QX(subs, Ptfo, Pcert0, PTls13) {
    ...
    if (server != "") {
      if (server.indexOf("@")!=-1) {
        ...
        var tfo = Ptfo == 1 ? "fast-open=true" : "fast-open=true";
...
function VQ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  var node = ""
  var ip = "vmess=" + server.split(",")[1].trim() + ":" + server.split(",")[2].trim() + ", " + "method=chacha20-ietf-poly1305, " + "password=" + server.split(",")[4].split("\"")[1] + ", "
  var tag = "tag=" + server.split("=")[0]
  var tfo = subs.indexOf("tfo=1") != -1 ? "fast-open=true, " : "fast-open=true, "
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  var node = ""
  var ip = "vmess=" + server.split("@")[1] + ", " + "method=chacha20-ietf-poly1305, " 
...
function S5R2QX(cnt,tlsp="false") {
  ...
  if (server != "") {
      ...
      var cert = Pcert0 != 0 ? "tls-verification=true" : "tls-verification=false";
      cert = tls == ""? "":cert
      var tfo = Ptfo0 == 1 ? "fast-open=true" : "fast-open=true";
...
function V2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (server != "") {
    ...
    pwd = "password=" + ss.id;
    mtd = "method=chacha20-ietf-poly1305"
    try {
      tag = "tag=" + decodeURIComponent(ss.ps);
    } catch (e) {
      tag = "tag=" + ss.ps;
    }
    udp = Pudp == 1 ? "udp-relay=true" : "udp-relay=true";
    tfo = Ptfo == 1 ? "fast-open=true" : "fast-open=true";
...
function SSR2QX(subs, Pudp, Ptfo) {
    ...
    if (cnt.split(":").length <= 8) { //排除难搞的 ipv6 节点
        ...
        pudp = Pudp == 1 ? "udp-relay=true" : "udp-relay=true";
        ptfo = Ptfo == 1 ? "fast-open=true" : "fast-open=true";
...
function TJ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
    ...
    pudp = (Pudp == 1 || cnt.indexOf("udp=1")!=-1) ? "udp-relay=true" : "udp-relay=true";
    ptfo = (Ptfo == 1 || cnt.indexOf("tfo=1")!=-1)? "fast-open=true" : "fast-open=true";
...
//🇷🇺 俄罗斯 GIA = custom, ip, 152, chacha20-ietf-poly1305, password123, https://xxx/download/SSEncrypt.module, obfs=tls, obfs-host=xxx.windows.com, udp-relay=true
function SCT2QX(content) {
    ...
    var ptfo = paraCheck(cnt, "tfo") == "true" ? "fast-open=true" : "fast-open=true";
    var pudp = paraCheck(cnt, "udp-relay") == "true" ? "udp-relay=true" : "udp-relay=true";
...
function SVmess2QX(content) {
    ...
    var pmtd = "method=chacha20-ietf-poly1305";
...
function CSS2QX(cnt) {
  ...
  udp = cnt.udp ? "udp-relay=true" : "udp-relay=true"
  uot = cnt["udp-over-tcp"] ?  "udp-over-tcp=true" : "udp-over-tcp=false"
  tfo = cnt.tfo ? "fast-open=true" : "fast-open=true"
...
function CV2QX(cnt) {
  ...
  mtd = "method="+ "chacha20-ietf-poly1305" //cnt.cipher
  udp = cnt.udp ? "udp-relay=true" : "udp-relay=true" //暂不支持
  tfo = cnt.tfo ? "fast-open=true" : "fast-open=true"
  obfs = ""
...
function XUDP(cnt,pudp) {
  var udp = pudp == 1 && /^(shadowsocks|trojan|vmess)/.test(cnt.trim()) ? "udp-relay=true, " : "udp-relay=true, "
  if(cnt.indexOf("udp-relay") != -1){
